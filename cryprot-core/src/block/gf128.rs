...
mod clmul {
    ...
    mod benches {
        ...
        fn bench_gf128_mul(b: &mut Bencher) {
            let [low, high] = unsafe { transmute(thread_rng().gen::<[u128; 2]>()) };
...
mod scalar {
    ...
    /// Generated by ChatGPT o3-mini and reviewed by me.
    ...
    mod benches {
        ...
        #[bench]
        fn bench_gf128_mul(b: &mut Bencher) {
            let [low, high] = thread_rng().gen::<[u128; 2]>();
...
mod tests {
    ...
    fn test_clmul128() {
        for _ in 0..1000 {
            let (a, b) = thread_rng().gen::<(u128, u128)>();
            unsafe {
