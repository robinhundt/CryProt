...
use seec_core::buf::Buf;

fn criterion_benchmark(c: &mut Criterion) {
    let num_ots = 2_usize.pow(18);
    let code = ExConvCode::new(num_ots as u64);

    let mut data = Vec::zeroed(code.conf().code_size as usize);
    ...
    let mut libote_code = libote::ExConvCode::new(
        num_ots as u64,
        code.conf().code_size,
        code.conf().expander_weight,
        code.conf().accumulator_size,
    );
    g.bench_function("dual_encode blocks", |b| {
        b.iter(|| {
