...
pub fn transpose_bitmatrix_into(input: &[u8], output: &mut [u8], rows: usize) {
    ...
    assert_eq!(0, input.len() % chunk_size);
    let chunk_cols_byte = chunk_size / rows;
    ...
    for chunk in 0..chunks {
        unsafe {
            ...
            while row <= rows - 16 {
                let mut col = 0;
                while col < cols {
                    let mut v = load_bytes(input, row, col, cols, chunk, chunk_cols_byte);
                    ...
                    for i in (0..8).rev() {
                        ...
                        let idx = out(row, col + i, rows, chunk) as isize;
...
#[inline]
fn inp(x: usize, y: usize, cols: usize, chunk: usize, chunk_cols_byte: usize) -> usize {
    x * cols / 8 + y / 8 + chunk * chunk_cols_byte
}
#[inline]
fn out(x: usize, y: usize, rows: usize, chunk: usize) -> usize {
    y * rows / 8 + x / 8 + chunk * rows
}
...
unsafe fn load_bytes(
    ...
    unsafe {
        ...
        {
            let v = _mm_setr_epi8(
                *b.get_unchecked(inp(row, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 1, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 2, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 3, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 4, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 5, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 6, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 7, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 8, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 9, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 10, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 11, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 12, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 13, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 14, col, cols, chunk, chunk_size)) as i8,
                *b.get_unchecked(inp(row + 15, col, cols, chunk, chunk_size)) as i8,
            );
            transmute(v)
        }
...
mod tests {
    ...
    fn arbitrary_bitmat(max_row: usize, max_col: usize) -> BoxedStrategy<(Vec<u8>, usize, usize)> {
        (
            (128..=max_row).prop_map(|row| row / 16 * 16),
            (1024 * 1024 / 128 * 8..=max_col).prop_map(|col| col / 16 * 16),
        )
    ...
    proptest! {
        #[cfg(not(miri))]
        #[test]
        fn test_double_transpose((v, rows, cols) in arbitrary_bitmat(128, 1024 * 1024 / 128 * 8)) {
